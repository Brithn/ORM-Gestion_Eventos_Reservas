...
const {
  ...
  getAsistenteSchema
} = require('./../schemas/asistente.schema');
...
router.get(
  '/:id',
  validatorHandler(getAsistenteSchema, 'params'),
  async (req, res, next) => {
    try {
      const { id } = req.params;
...
router.post(
  '/',
  validatorHandler(createAsistenteSchema, 'body'),
  async (req, res, next) => {
    try {
      const body = req.body;
...
router.patch(
  '/:id',
  validatorHandler(getAsistenteSchema, 'params'),
  validatorHandler(updateAsistenteSchema, 'body'),
  async (req, res, next) => {
    try {
      const { id } = req.params;
      const body = req.body;
